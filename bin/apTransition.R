...
if (length(args) < 3) {
  stop("Usage: apTransition.r <inputTable> <outputTable> <variantCaller>", call.=FALSE)
}
...
#print(paste(as.character(transi_snv$ID[1]),variantCaller,sep="_")

table_transi=data.frame(cbind(ID=paste(as.character(transi_snv$ID[1]),variantCaller,sep="_"),t(as.matrix(table_transi)),indels))
